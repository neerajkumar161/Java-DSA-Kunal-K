...
const createTargetArrayTS = function (nums, index) {
  const arr = []
  nums.map((el: number, idx: number) => {
    const indexToInsert = index[idx]
    arr.splice(indexToInsert, 0, el as never)
  })
  return arr
}

createTargetArrayTS([0, 1, 2, 3, 4], [0, 1, 2, 2, 1])
